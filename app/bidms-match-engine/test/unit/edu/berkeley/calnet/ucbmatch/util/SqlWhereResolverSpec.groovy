...
class SqlWhereResolverSpec extends Specification {
    ...
    def "test canonical sql generation"() {
        ...
        EXACT     | [caseSensitive: true, alphanumeric: true]              | "regex_replace(SOR,'[^A-Za-z0-9]','','g')=?"            | 'SIS123'
        SUBSTRING | [caseSensitive: true, alphanumeric: true]              | "regex_replace(SOR,'[^A-Za-z0-9]','','g')=?"            | 'SIS123'
        EXACT     | [alphanumeric: true]                                   | "regex_replace(lower(SOR),'[^A-Za-z0-9]','','g')=?"     | 'sis123'
        SUBSTRING | [alphanumeric: true]                                   | "regex_replace(lower(SOR),'[^A-Za-z0-9]','','g')=?"     | 'sis123'
    ...
    def "test potential sql generation"() {
        ...
        EXACT     | [caseSensitive: true, alphanumeric: true]                           | "regex_replace(SOR,'[^A-Za-z0-9]','','g')=?"            | 'SIS123'
        SUBSTRING | [caseSensitive: true, alphanumeric: true]                           | "regex_replace(SOR,'[^A-Za-z0-9]','','g')=?"            | 'SIS123'
        DISTANCE  | [caseSensitive: true, alphanumeric: true]                           | "regex_replace(SOR,'[^A-Za-z0-9]','','g')=?"            | 'SIS123'
        EXACT     | [alphanumeric: true]                                                | "regex_replace(lower(SOR),'[^A-Za-z0-9]','','g')=?"     | 'sis123'
        SUBSTRING | [alphanumeric: true]                                                | "regex_replace(lower(SOR),'[^A-Za-z0-9]','','g')=?"     | 'sis123'
        DISTANCE  | [alphanumeric: true]                                                | "regex_replace(lower(SOR),'[^A-Za-z0-9]','','g')=?"     | 'sis123'
